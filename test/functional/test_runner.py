...
def main():
    ...
    tmpdir = "%s/pesium_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and pesiumd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "pesiumd"]) is not None:
            print("%sWARNING!%s There is already a pesiumd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "PESIUMD" not in os.environ:
        os.environ["PESIUMD"] = build_dir + '/src/pesiumd' + exeext
        os.environ["PESIUMCLI"] = build_dir + '/src/pesium-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `pesium-cli help` (`rpc_interface.txt`).
